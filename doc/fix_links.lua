...
assert(FORMAT == "gfm" and PANDOC_STATE.output_file == "README.md")

function Link(el)
    if el.target:match"%.md$" and not el.target:match"^http" then
...
function Image(el)
    if el.src:match"%.svg$" and not el.src:match"^http" then
